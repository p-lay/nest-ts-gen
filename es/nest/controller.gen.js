...
class ControllerGen extends baseRender_1.BaseRender {
    ...
    renderImports() {
        return `import { Controller, Get, Post, Body, Req, Res } from '@nestjs/common'\nimport { ${this.serviceName} } from '../${this.config.serviceFolderName}/${this.modelKey}.service'\nimport { ${this.commonResType} } from '${this.config.sourceContractFolderRelativePath}/${this.globalTypeFileName}'\nimport { ${this.allDtoTypes.join(", ")} } from '${this.config.sourceContractFolderRelativePath}/${this.modelKey}'`;
    }
    renderClass() {
        const decorator = this.renderClassDecorator();
        return `${decorator}\nexport class ${this.className} {${this.addLine(1)}constructor(private readonly service: ${this.serviceName}) {}${this.addLine(1)}${this.renderMethods()}\n}`;
    }
    renderMethods() {
        let methodStr = "";
    ...
    renderMethod(key, method) {
        return `${this.addLine(1)}@Post('${key}')${this.addLine(1)}async ${key}(@Body() ${this.mappingConfig.paramName}: ${this.getRequestType(method.req)}): ${this.getControllerResponseType(method.res)} {${this.addLine(2)}const data = await this.service.${key}(${this.mappingConfig.paramName})${this.addLine(2)}return {${this.addLine(3)}data,${this.addLine(2)}}${this.addLine(1)}}`;
    }
    ...
    generate() {
        for (const modelKey in this.mapping) {
            ...
            if (!modelConfig.disableController) {
                ...
                fs.writeFileSync(`${this.config.controllerOutFolder}/${modelKey}.controller.ts`, str);
            }
            new service_gen_1.ServiceGen({
                modelKey,
                model,
                modelConfig,
...
exports.ControllerGen = ControllerGen;
